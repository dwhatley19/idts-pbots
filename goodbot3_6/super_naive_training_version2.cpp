...
string Training::get_action(vector<string> hole, vector<string> table, vector<string> legal_actions)
{
    ...
    if(discard == 1) {
        ...
        if(double(action)/100 < cur.checkfold / total_prob && !cantfold && check == 1) {
            if(check == 1) {
                //cout << "checkfolding\n";
                // 0 = CHECK
                b.actions.push_back(fttoi(0, hs, u, b.current_round));
        ...
        else if(double(action)/100 < (cur.checkfold + cur.betlow) / total_prob) {
            //cout << "bet high\n";
            if(pre != "") {
...
{   
    ...
    for(int i = 0; i < NUM_STATES; ++i) {
        s[i].checkfold = max(0.0, s[i].checkfold);
        s[i].betlow = max(0.0, s[i].betlow);
        s[i].bethigh = max(0.0, s[i].bethigh);
    }
